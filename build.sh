...
evidence=$(mktemp -d)
python3 "$root/tools/image_metadata.py" --root "$root" --repository edbfi/otpravkarr-docker --branch "$channel" --revision "$(git -C "$root" rev-parse HEAD)" --arch "$arch" --output "$evidence"
args=()
